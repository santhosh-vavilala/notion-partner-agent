...
async def classify(state: AgentState):
    return {"route": await llm.classify(state["message"]), "tool_calls": [], "tool_results": [], "approval_required": False}
...
async def plan_tools(state: AgentState):
    calls = await llm.select_tools(state["message"], state["route"], state["tools"])
    known={t["name"] for t in state["tools"]}
    calls=[c for c in calls if c["name"] in known][:settings.max_mcp_tool_calls]
...
def build_graph(checkpointer=None):
    ...
    g.add_edge("general",END); g.add_edge("discover_tools","plan_tools"); g.add_edge("plan_tools","approval_gate")
    g.add_conditional_edges("approval_gate", after_approval, {END:END,"execute_tools":"execute_tools"})
    g.add_edge("execute_tools","synthesize"); g.add_edge("synthesize",END)
